...
def prepare_sample_hdf(schema, hdf_path, max_table_data, sample_size):
    ...
    # logger.info('table meta_data is {}'.format(prep.table_meta_data))
    ...
    # logger.info('df_sample_cache and df_full_samples for {} are {} \n {}'.format(start_table, df_sample_cache, df_full_samples))
        ...
        for relationship_obj in schema.relationships:
            if (relationship_obj.start in sampled_tables and not relationship_obj.end in sampled_tables) or (
                ...
                if relationship_obj.start in sampled_tables and not relationship_obj.end in sampled_tables:
                    ...
                    # logger.info('relationship_obj is {}'.format(relationship_obj))

                    # logger.info('relationship_obj.end and relationship_obj.end_attr are {} \t {}'.format(relationship_obj.end, relationship_obj.end_attr))
                    # logger.info('relationship_obj.start and relationship_obj.start_attr are {} \t {}'.format(relationship_obj.start, relationship_obj.start_attr))

                    # logger.info("Left attribute is: {}".format(left_attribute))
                    # logger.info("Right attribute is: {}".format(right_attribute))


                    df_samples = df_sample_cache[relationship_obj.end]
                    # logger.info('df_samples (near the error place) is \n{}'.format(df_samples))
                    # logger.info('next_table)data (near the error place) is \n{}'.format(next_table_data))
                    df_samples = df_samples.set_index(left_attribute, drop=False)
                    df_samples.index.name = None
                    next_table_data = next_table_data.set_index(right_attribute, drop=False)

                    # logger.info('df_samples contains : \n{}'.format(df_samples))
                    # logger.info('next_table_data contains : \n{}'.format(next_table_data))

                    next_table_data = df_samples.merge(next_table_data, right_index=True, left_on=left_attribute)
                    # logger.info('the merged dataframe is \n{}'.format(next_table_data))
                    # only keep rows with join partner
                    next_table_data = next_table_data[
                        next_table_data[relationship_obj.end + '.' + relationship_obj.multiplier_attribute_name] > 0]
