...
def identity_likelihood_range(node, data, dtype=np.float64, overwrite_ranges=None, **kwargs):
    ...
    logger.info("The node is : \n{} \nAnd node.scope is : \n{} \nAnd data is : \n{}".format(node, node.scope, data))

    if node.scope[0]==7:
        node.scope[0]=8
        logger.info("Updated node.scope is : \t{}".format(node.scope))
    ...
    logger.info("The data in ranges is : \n{}".format(ranges))
    ...
    for i, rang in enumerate(ranges):
        ...
        # logger.info("The ranges in NominalRange object are : \n{}".format(rang.get_ranges()))
