...
def build_ds_context(column_names, meta_types, null_values, table_meta_data, no_compression_scopes, train_data,
    ...
    for col in range(train_data.shape[1]):
        ...
        logger.info('feature_meta_type is \t{}'.format(feature_meta_type))
        ...
        if feature_meta_type == MetaType.REAL:
            ...
            # logger.info('column_names and unified_column_dictionary are \n{} \n and \n{}'.format(column_names, unified_column_dictionary))

            # Remove this line if causing bugs...... so basically the discrete metatype was causing a lot of bugs.......removing it entirely for now
            meta_types[col] = MetaType.REAL
            ...
            if column_names is not None \
                ...
                domain.append((np.linspace(0, no_diff_values + 1, 2)).tolist())
            else:
                # logger.info('domain_values are \n{}'.format(domain_values))

                domain.append((np.linspace(domain_values[0], domain_values[1] + 1, 2)).tolist())
        else:
            raise Exception("Unkown MetaType " + str(meta_types[col]))

    logger.info('the domain attribute contains \n{}'.format(domain))
